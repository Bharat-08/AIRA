...
def parse_resume_text(text: str) -> dict:
    ...
    """
    try:
        # Configure the Gemini client with your key from settings
        genai.configure(api_key=settings.GEMINI_API_KEY)
        
        # Initialize the generative model
        model = genai.GenerativeModel('gemini-1.5-flash')

        # The prompt is adapted for Gemini but keeps the same extraction goals
...
---"""
        ...
        # We raise an exception to be handled by the calling function
